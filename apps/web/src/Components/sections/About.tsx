    ...
    <section id="about" className="py-20 sm:py-24 px-6 bg-white">
      <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        <div className="gsap-reveal">
          ...
          <h2
            className="text-3xl sm:text-5xl font-bold text-zinc-900 tracking-tight mb-6 leading-tight"
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          {stats.map((stat, i) => (
            <motion.div
              ...
              className="rounded-2xl border border-zinc-100 bg-zinc-50/60 p-5 sm:p-6 hover:border-violet-100 hover:bg-violet-50/30 transition-colors"
            >
              <div className="text-2xl sm:text-3xl font-bold text-zinc-900 mb-1">{stat.value}</div>
              <div className="text-[13px] sm:text-sm text-zinc-500 leading-snug">{stat.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
